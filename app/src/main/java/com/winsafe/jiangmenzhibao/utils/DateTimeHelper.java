...
public class DateTimeHelper {
	...
	public static Date getTheFirstDayOfMonth(String yyyymm) {
		...
		return DateTimeHelper.getDateTime(dateStr);
	}
	...
	public static Date[][] makeCalendar(int year, int month) {
		...
		// Object[][]
		// 指定年月的第一天
		Date date = DateTimeHelper.getDate(year + "-" + month + "-01");
		// 次月的第一天
		Date lastDate = DateTimeHelper.addMonth2Date(1, date);
		// 第一天是周几
		int firstDayWeek = DateTimeHelper.getDayOfWeek(date);
		// 将星期日处理为一周的最后一天
		if (1 == firstDayWeek) {
			firstDayWeek = 8;
		}
		...
		// 遍历一个月，以周为循环生成二维数组
		while (DateTimeHelper.getDiffDays(date, lastDate) > 0) {
			if (col > 6) {
				row = row + 1;
				col = 0;
			}
			dateArray[row][col] = date;
			date = DateTimeHelper.addDay2Date(1, date);
			col++;
		}
	...
	public static Date[][] makeCalendar(Date startDate, Date endDate) {
		...
		// 开始日期是周几
		int firstDayWeek = DateTimeHelper.getDayOfWeek(startDate);
		...
		while (DateTimeHelper.getDiffDays(startDate, endDate) >= 0) {
			if (col > 6) {
				row = row + 1;
				col = 0;
			}
			dateArray[row][col] = startDate;
			startDate = DateTimeHelper.addDay2Date(1, startDate);
			col++;
		}
	...
	public static List<Date> getWeekEndList(Integer year) {
		Date date = DateTimeHelper.getDate(year + "-01-01");
		// 次年的第一天
		Date lastDate = DateTimeHelper.addYear2Date(1, date);
		List<Date> weekendList = new ArrayList<Date>();
		while (DateTimeHelper.getDiffDays(date, lastDate) > 0) {
			int dayOfweek = DateTimeHelper.getChinaDayOfWeek(date);
			if (6 == dayOfweek || 7 == dayOfweek) {
				// System.out.println(DateTimeHelper.formatDate("yyyy-mMM-dd",
				// date) +
				// ":" + dayOfweek);
