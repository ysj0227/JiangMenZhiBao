...
public class ReceiveOrganActivity extends AppBaseActivity {
    @BindView(R.id.etSearch)
    EditText etSearch;
    @BindView(R.id.lvListData)
    ListView lvListData;
    private List<OrganDataBean> list= new ArrayList<>();
    //筛选列表
    private List<OrganDataBean> mList=null;
    private OrganAdpter adpter;
    @Override
    ...
    protected void initView() {
        setHeader("选择收货机构", true, false, 0, "", null);
        getList();
    ...
    private void getList() {
        list= MyApp.organDB.findAllByWhere(OrganDataBean.class, " type=\"" +AppConfig.AUTHORITY_RECEIVE+ "\"");
        mList=list;
        adpter=new OrganAdpter(ReceiveOrganActivity.this,mList) ;
        lvListData.setAdapter(adpter);
