...
public class BillNotReturnActivity extends AppBaseActivity {
    ...
    private String FromOrganID = "";//"退货机构id
    private String ToOrganID = "";//调入机构id
    private String FromHouseID = "";//"退货仓库id
    private String ToHouseID = "";//调入仓库id
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setLayout(R.layout.layout_return_not_store);

    }

    @Override
    protected void initView() {
        setHeader("无单退货", true, false, 0, "", null);
        isDownData();
        ...
        Dialog dialog= new AlertDialog.Builder(this)
                ...
                .setPositiveButton("下载", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        openActivity(BillNotReturnActivity.this, DownLoadActivity.class,true);
                        dialog.dismiss();
                        return;
                    }
                })
    ...
    private void showDefaultData() {
        ...
        List<OrganDataBean> list = MyApp.organDB.findAllByWhere(OrganDataBean.class, " type=\"" + AppConfig.AUTHORITY_RECEIVE + "\"");
...
//            return;
        ...
        List<OrganDataBean> mlist = MyApp.organDB.findAllByWhere(OrganDataBean.class, " type=\"" + AppConfig.AUTHORITY_SEND + "\"");
    ...
    private void HouseSendDefault() {
        List<WarehouseDataBean> lists = MyApp.warehouseDB.findAllByWhere(WarehouseDataBean.class,
                " type=\"" + AppConfig.AUTHORITY_RECEIVE + "\"" + " and " + " organID=\"" + FromOrganID + "\"");
    ...
    private void HouseReceiveDefault() {
        List<WarehouseDataBean> mlists = MyApp.warehouseDB.findAllByWhere(WarehouseDataBean.class,
                " type=\"" + AppConfig.AUTHORITY_SEND + "\"" + " and " + " organID=\"" + toOrgID + "\"");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.rl1:
            case R.id.ivSendOrgan://"退货机构
                openActivityForResult(BillNotReturnActivity.this, ReceiveOrganActivity.class, RETURN_DATA_SEND_ORGAN);

                break;
            case R.id.rl2:
            case R.id.ivSendStore://"退货仓库
                if (StringHelper.isNullOrEmpty(tvSendOrgan.getText().toString())) {
                    Toast.makeText(this, "请选择退货机构", Toast.LENGTH_SHORT).show();
                    return;
                }
                Bundle bundle4 = new Bundle();
                bundle4.putString("ToOrganID", FromOrganID);
                openActivityForResult(BillNotReturnActivity.this, ReceiveHouseActivity.class, RETURN_DATA_SEND_STORE, bundle4);

                break;
            case R.id.rl3:
            case R.id.ivReceiveOrgan://调入机构
                openActivityForResult(BillNotReturnActivity.this, SendOrganActivity.class, RETURN_DATA_RECEIVE_ORGAN);

                break;
            case R.id.rl4:
            case R.id.ivReceiveStore://调入仓库
                if (StringHelper.isNullOrEmpty(tvReceiveOrgan.getText().toString())) {
                    Toast.makeText(this, "请选择调入机构", Toast.LENGTH_SHORT).show();
                    return;
                }
                Bundle bundle2 = new Bundle();
                bundle2.putString("FromOrganID", ToOrganID);
                openActivityForResult(BillNotReturnActivity.this, SendHouseActivity.class, RETURN_DATA_RECEIVE_STORE, bundle2);

                break;

            case R.id.btnReset://重置
                if (getListSize(MyApp.organDB, AppConfig.AUTHORITY_RECEIVE)>1){
                    tvSendOrgan.setText(null);
                    tvSendStore.setText(null);
                }
                if (getListSize(MyApp.organDB, AppConfig.AUTHORITY_SEND)>1){
            ...
            case R.id.btnScan://扫描
                ...
                bundle.putString(AppConfig.OPERATE_TYPE, AppConfig.OPERATE_TYPE_NOT_TH);
                ...
                openActivity(BillNotReturnActivity.this, CaptureActivity.class, bundle, false);
                break;
    ...
    private boolean returnNullToast() {
        ...
        if (StringHelper.isNullOrEmpty(tvSendStore.getText().toString())) {
            Toast.makeText(this, "请选择退货仓库", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (StringHelper.isNullOrEmpty(tvReceiveOrgan.getText().toString())) {
            Toast.makeText(this, "请选择调入机构", Toast.LENGTH_SHORT).show();
        ...
        if (StringHelper.isNullOrEmpty(tvReceiveStore.getText().toString())) {
            Toast.makeText(this, "请选择调入仓库", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                case RETURN_DATA_SEND_ORGAN://"退货机构
                    tvSendOrgan.setText(name);
                    fromOrgName = name;//名称
                    fromOrgID = id;//id
                    ...
                    HouseSendDefault();
                    break;
                case RETURN_DATA_SEND_STORE://"退货仓库
                    ...
                    FromHouseID = id;
                    break;
                case RETURN_DATA_RECEIVE_ORGAN://调入机构
                    ...
                    HouseReceiveDefault();
                    break;
                case RETURN_DATA_RECEIVE_STORE://调入仓库
