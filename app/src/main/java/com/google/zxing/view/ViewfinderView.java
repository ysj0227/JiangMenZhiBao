...
public final class ViewfinderView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			if ((CaptureActivity.mCurrentState != null) && (CaptureActivity.mCurrentState.equals("onecode"))) {
				canvas.drawText(getResources().getString(R.string.textview_scanprompt), frame.left + 8, (float) (frame.bottom + (float) 10 * density), paint);

			}
			if ((CaptureActivity.mCurrentState != null) && (CaptureActivity.mCurrentState.equals("qrcode"))) {
//				canvas.drawText(getResources().getString(R.string.textview_scanprompt), frame.left - 20, (float) (frame.bottom + (float) 40 * density), paint);

				// 将提醒文本方知道扫描框的上部
//				canvas.drawText(getResources().getString(R.string.textview_scanprompt), frame.left, (float) (frame.top - (float) 10 * density), paint);
				float textWidth = paint.measureText(getResources().getString(R.string.textview_scanprompt));
				canvas.drawText(getResources().getString(R.string.textview_scanprompt), (width - textWidth) / 2, (float) (frame.top - (float) 20 * density), paint);
			}
